...
function E4000(i2c) {
	...
	function setMux(freq) {
			// set the band
		return i2c.writeReg(E4K_REG_BIAS, 3)
	...
	function setPll(freq) {
		...
		return i2c.writeReg(E4K_REG_SYNTH7, range.REG)
			// program Z
	...
	function getPllLock() {
		return i2c.readReg(E4K_REG_SYNTH1)
			.then(value => {
				if (value & 1)
	...
	function initDevice() {
		...
		return i2c.readReg(0)
			// Make sure we reset everything and clear POR indicator
			.then(() => i2c.writeReg(E4K_REG_MASTER1,
			...
			.then(() => i2c.writeRegMask(E4K_REG_AGC1, E4K_AGC_MOD_SERIAL, E4K_AGC1_MOD_MASK))
			// Set Mixer Gain Control to manual
			.then(() => i2c.writeRegMask(E4K_REG_AGC7, 0x00, E4K_AGC7_MIX_GAIN_AUTO))
	...
	function setAutoGain() {
		...
		return i2c.writeRegMask(E4K_REG_AGC1, E4K_AGC_MOD_IF_SERIAL_LNA_AUTON, E4K_AGC1_MOD_MASK)
