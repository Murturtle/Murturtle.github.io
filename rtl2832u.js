...
function RTL2832U(usb) {
	...
	function powerUp() {
		return usb.open()
			...
			.then(() => {
				return i2c_readReg(E4K_I2C_ADDR, E4K_CHECK_ADDR)
					.then(data => {
						if (data != E4K_CHECK_VAL)
			...
			.then(found => {
				if (found)
					return found;
				return i2c_readReg(R820T_I2C_ADDR, R82XX_CHECK_ADDR)
					.then(data => {
						if (data != R82XX_CHECK_VAL)
							return Promise.resolve(false);
						tuner = new R820T(new I2C(R820T_I2C_ADDR));
							// disable Zero-IF mode
						return demodWriteReg(1, 0xb1, 0x1a)
			...
			.then(disableI2C)
			.then(() => "Connected to Realtek RTL2832U with " + tuner.getName() + " tuner");
	}
	...
	function setFir(fir) {
		...
		for (let i = 0; i < 8; ++i) {
			let val = fir[i];
			if (val < -128 || val > 127)
				return Promise.reject("8 bit signed FIR filter coefficients out of bounds");
			tmp[i] = val;
		}
		for (let i = 0; i < 8; i += 2) {
			let val0 = fir[8+i];
			...
			if (val0 < -2048 || val0 > 2047 || val1 < -2048 || val1 > 2047)
				return Promise.reject("12 bit signed FIR filter coefficients out of bounds");
	...
	function resetDemod() {
		...
		return demodWriteReg(1, 0x01, 0x14)
			.then(() => demodWriteReg(1, 0x01, 0x10));
	}

	function setCenterFrequency(freq) {
		return enableI2C()
			.then(() => tuner.setFrequency(freq + int_freq))
			.then(actual => disableI2C()
				.then(() => actual - int_freq));
	}

	function resetBuffer() {
		return writeReg(CMD_USB_WR, USB_EPA_CTL, 0x0210, 2)
			.then(() => writeReg(CMD_USB_WR, USB_EPA_CTL, 0x0000, 2));
	}

	function readSamples(len) {
		return usb.transferIn(1, len * 2)
			.then(e => e.data.buffer);
	}
	...
	function readArray(block, reg, len) {
		...
		return usb.controlTransferIn(ti, len)
			.then(e => e.data.buffer);
	}
	...
	function readReg(block, reg, len) {
		if (len == undefined)
			len = 1;
		return readArray(block, reg, len)
			.then(data => le2val(data));
	}
	...
	function demodWriteReg(page, addr, value, len) {
		if (len == undefined)
			len = 1;
		return writeArray(page | 16, (addr << 8) | 0x20, val2be(value, len))
			.then(() => demodReadReg(0x0a, 0x01));
	}
	...
	function i2c_read(addr, reg, len) {
		return writeArray(CMD_I2C_DA_WR, addr, new Uint8Array([reg]).buffer)
			.then(() => readArray(CMD_I2C_DA_RD, addr, len));
	}

	function i2c_readReg(addr, reg) {
		return writeArray(CMD_I2C_DA_WR, addr, new Uint8Array([reg]).buffer)
			.then(() => readReg(CMD_I2C_DA_RD, addr));
	}
